...
- **F006** — Profile feature (`features/profile`)
- **F007** — Notifications feature (`features/notifications`)
...
| SCR006 | Profile Tab | `/profile` | F006 | Implemented |
...
| SCR008 | Notifications | `/notifications` | REG001 header bell icon | F007 | Implemented |
...
**SCR008 — Notifications** (`NotificationsScreen` via `NotificationsRouteWrapper`, `lib/core/router/notifications_route_wrapper.dart`):
F007 — notifications list screen. Pure widget (props-driven); `NotificationsRouteWrapper` binds `notificationsControllerProvider`, handles loading/error states, resolves deep-links. Mark-read per item or mark-all-read. Deep-links: kudos→`/kudos/detail/:id`, award→`selectedAwardIdProvider`+`/awards`, secretBox→`/secret-box`, system→no-op. `notificationsUnreadCountProvider` is single source of truth for Home + Awards bell badge. MoMorph: `_b68CBWKl5`. vi/en/ja i18n (5 keys). Deferred: real push/FCM, read-state persistence, relative-time i18n, pagination.
...
**SCR017 — Self Profile** (`ProfileScreen` via `SelfProfileRouteWrapper`, `lib/core/router/profile_route_wrappers.dart`):
F006 — self profile tab (shell branch 3). MoMorph: `hSH7L8doXB`. `isSelf = true` — shows KudosStats section (received/sent/hearts + Secret Box entry), edit-profile button, settings icon. `profileProvider(currentUserId)` loads `ProfileData`. `ProfileFilterHost` owns `KudosFilter{received,sent}` client-side filter. Local stub. Edit-profile submission + settings content deferred. vi/en/ja i18n (loading/error keys).

**SCR018 — Other Profile** (`ProfileScreen` via `OtherProfileRouteWrapper`, `lib/core/router/profile_route_wrappers.dart`):
F006 — other-user profile, standalone push outside shell. Route `/profile/:userId`. MoMorph: `bEpdheM0yU`. `isSelf = false` — shows Send-Kudo button instead of KudosStats; back arrow in top bar. Same `ProfileFilterHost` + `ProfileScreen` as SCR017 (single widget, two variants). `onTapUser` → `push(/profile/:userId)` for avatar taps. Local stub.
...
| SCR006 | Profile Tab | `/profile` | F006 | Yes | Yes |
| SCR007 | Search | `/search` | F002 | No | Placeholder |
| SCR008 | Notifications | `/notifications` | F007 | No | Yes |
...
| SCR017 | Self Profile | `/profile` | F006 | Yes | Yes |
| SCR018 | Other Profile | `/profile/:userId` | F006 | No | Yes |
